...
AnimatedModel::AnimatedModel(const std::string &filename) :
	mView(glm::mat4(1.0f)),
	mProjection(glm::mat4(1.0f))
{
	loadModel(filename);
...
void AnimatedModel::LoadFromFile(const std::string &filename)
{
	loadModel(filename);
}

void AnimatedModel::AddMesh(AnimatedMesh mesh)
{
	mMeshes.push_back(mesh);
}

void AnimatedModel::ClearMeshes()
{
	mMeshes.clear();
}
...
void AnimatedModel::Draw(std::vector<Material> &materials)
{
	for (unsigned int i = 0; i < mMeshes.size(); i++)
	{
		if (materials.size() != 0)
...
void AnimatedModel::SetRotation(glm::quat rotation)
{
	mRotation = rotation;
}
...
void AnimatedModel::SetView(glm::mat4 view)
{
	mView = view;
}

void AnimatedModel::SetProjection(glm::mat4 projection)
{
	mProjection = projection;
}

glm::mat4 AnimatedModel::GetModel() const
{
...
glm::mat4 AnimatedModel::GetView() const
{
	return mView;
}

glm::mat4 AnimatedModel::GetProjection() const
{
	return mProjection;
}

void AnimatedModel::SetViewPosition(glm::vec3 position)
{
	mViewPosition = position;
}

void AnimatedModel::SetLights(std::vector<entityx::Entity> lights)
{
...
void AnimatedModel::SetLights(std::vector<entityx::Entity> lights, std::vector<Material> &materials)
{
...
AnimatedMesh AnimatedModel::processMesh(aiMesh *mesh, const aiScene *scene)
{
	...
	for (unsigned int i = 0; i < mesh->mNumBones; i++) {
		...
		if (mBoneMapping.find(boneName) == mBoneMapping.end()) {
			boneIndex = mBoneMapping.size() + 1;
			mBoneInfoList.push_back(BoneInfo());
		}
		else {
		...
		mBoneInfoList[boneIndex].BoneOffset = mat4_cast(mesh->mBones[i]->mOffsetMatrix);
		...
		for (unsigned int j = 0; j < mesh->mBones[i]->mNumWeights; j++) 
		{
			...
			bones[vertexID].IDs[j] = boneIndex;
			bones[vertexID].Weights[j] = weight;
		}
	}

	return AnimatedMesh(vertices, indices, bones);
}

void AnimatedModel::SetFogParams(FogComponent &fog)
{
	mFog = fog;
}